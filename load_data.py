                ...
                for news in os.listdir(non_rumor_path):
                    ...
                    if not news.startswith('._') and news != '.DS_Store':
                        if len(os.listdir(os.path.join(event_path, 'non-rumours', news, 'reactions'))) == 0:
                            continue
                        source_tweets_path = os.path.join(event_path, 'non-rumours', news, 'source-tweets', f'{news}.json')
                        try:
                                    ...
                                    try:
                                        ...
                                        if comment_data:
                                            ...
                                            if comment_data["in_reply_to_status_id"] == None:
                                                ...
                                                content["parent"] = node_index[int(comment_data["in_reply_to_status_id"])]
                                            else:
                                                content["parent"] = 0
...
class HugeDataset(Dataset):
    def __init__(self, root, centrality_metric="Pagerank", undirected=False, transform=None, pre_transform=None, pre_filter=None):
        self.undirected = undirected
        self.centrality_metric = centrality_metric
        self.tokenizer = XLMRobertaTokenizer.from_pretrained('xlm-roberta-base', cache_dir="./")
        self.model = XLMRobertaModel.from_pretrained('xlm-roberta-base', cache_dir="./")
...
def load_datasets_with_prompts(args):
    ...
    dataset_mapping = {
        "DRWeiboV3": {"dataset": TreeDataset("./Data/DRWeiboV3/"), "prompt_key": "en_prompt"},
        "Weibo": {"dataset": TreeDataset("./Data/Weibo/"), "prompt_key": "en_prompt"},
        "WeiboCOVID19": {"dataset": CovidDataset("./Data/Weibo-COVID19/Weibograph"), "prompt_key": "en_prompt"},
        "PHEME": {"dataset": TreeDataset_PHEME("./Data/pheme/"), "prompt_key": "en_prompt"},
        "Politifact": {"dataset": TreeDataset_UPFD("./Data/politifact/"), "prompt_key": "en_prompt"},
        "Gossipcop": {"dataset": TreeDataset_UPFD("./Data/gossipcop/"), "prompt_key": "en_prompt"},
        "TwitterCOVID19": {"dataset": CovidDataset("./Data/Twitter-COVID19/Twittergraph"), "prompt_key": "en_prompt"},
        # "Twitter15-tfidf": {"dataset": TreeDataset("./Data/Twitter15-tfidf/"), "prompt_key": "en_prompt"}
    }
